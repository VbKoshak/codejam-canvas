...
let drawsquare = (x, y, color) => {
    let isRGB = Array.isArray(color);
    let colorer;
    if (isRGB) {
        colorer = "rgba(" + color.join(',') + ((color.length == 4) ? ")" : ",1)");
    } else {
        colorer = "#" + color;
    }
    let size = options.squareSize;
...
let init = (size = 32) => {
    ...
    changeSize(size);
    drawImage(options.basicInstructions[size]);
};
